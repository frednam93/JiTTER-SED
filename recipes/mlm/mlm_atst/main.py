...
from recipes.mlm.mlm_passt.train import MLMTrainer
...
def main(iteration=None):
    ...
    #set logger
    logger = get_logger(configs["generals"]["save_folder"], log_level=configs["generals"]["log_level"])
    #torch information
    ...
    patch_transformer_params = list(map(id, net.get_encoder().parameters()))
    base_params = filter(lambda p: id(p) not in patch_transformer_params, net.parameters())

    optim_kwargs = {"betas": (0.9, 0.999), "eps": 1e-8, "weight_decay": 1e-4}
    optimizer = optim.AdamW([{"params": base_params, 'lr': configs['opt']['lr_big']}], **optim_kwargs)
