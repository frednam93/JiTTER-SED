...
class Trainer:
    ...
    def __init__(self, optimizer, my_logger: Logger, net, ema_net, scheduler, encoder: Encoder, train_loader,
                 val_loader, test_loader, config, device, multigpu):
    ...
    def preprocess(self, wav, label, strong_mask, weak_mask, unlabel_mask):
        ...
        if self.multigpu:
            ...
            mel = self.net.get_feature_extractor()(wav)
    ...
    def train(self, epoch):
        ...
        for i, batch in enumerate(tk):
            ...
            if self.multigpu:
                ...
                stu_pred = self.net(stu_mel, **self.config[self.net.get_model_name()]["train_stu_kwargs"])
            ...
            with torch.no_grad():
                if self.multigpu:
                    ...
                    tch_pred = self.ema_net(tch_mel, **self.config[self.net.get_model_name()]["train_tch_kwargs"])
        ...
        with torch.no_grad():
            ...
            for batch in tk:
                ...
                if self.multigpu:
                    ...
                    stu_pred = self.net(mel, pad_mask=pad_mask, **self.config[self.net.get_model_name()]["val_kwargs"])
                stu_pred = self.Pred(*stu_pred)
                ...
                if self.multigpu:
                    ...
                    tch_pred = self.ema_net(mel, pad_mask=pad_mask, **self.config[self.net.get_model_name()]["val_kwargs"])
                tch_pred = self.Pred(*tch_pred)
    ...
    def test(self):
        ...
        _, _ = self.psds1(score_buffer.raw_student,
                          test_tsv,
                          test_dur,
                          save_dir=os.path.join(psds_folders[0], "unprocessed"))
        _, _ = self.psds1(score_buffer.raw_teacher,
                          test_tsv,
                          test_dur,
                          save_dir=os.path.join(psds_folders[1], "unprocessed"))
        stud_psds1, stud_psds1_single = self.psds1(score_buffer.post_student,
                                                   test_tsv,
                                                   test_dur,
                                                   save_dir=os.path.join(psds_folders[0], "postprocessed"))
        ...
        tch_psds1, tch_psds1_single = self.psds1(score_buffer.post_teacher,
                                                 test_tsv,
                                                 test_dur,
                                                 save_dir=os.path.join(psds_folders[1], "postprocessed"))
        ...
        score_file_name = f"sPSDS1={stud_psds1:>.4},sPSDS2={stud_psds2:>.4},tPSDS1={tch_psds1:>.4},tPSDS2={tch_psds2:>.4}.txt"
        with open(os.path.join(self.config["generals"]["save_folder"], score_file_name), "w", encoding="utf-8") as file:
            file.write("student class-wise psds1")
            for key, value in stud_psds1_single.items():
                file.write(f"{key}: {value}\n")
            file.write("teacher class-wise psds1")
            for key, value in tch_psds1_single.items():
                file.write(f"{key}: {value}\n")
        return
