...
from recipes.finetune.passt.setting import *
from recipes.finetune.passt.passt_train import PasstTrainer
...
if __name__ == "__main__":
    ...
    # set network
    net, ema_net = get_models_passt(configs)
    ...
    ##############################                TRAIN/VALIDATION                ##############################
    trainer = PasstTrainer(optimizer=optimizer,
                           my_logger=my_logger,
                           net=net,
                           ema_net=ema_net,
                           config=configs,
    ...
    if not configs['generals']["test_only"]:
        ...
        if ("finetune_mlm" in configs['generals'].keys()) and configs['generals']["finetune_mlm"]:
            ...
            params = {k: v for k, v in params_dict.items() if "classifier." not in k and "at_adpater" not in k}
