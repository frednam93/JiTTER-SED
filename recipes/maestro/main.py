...
root = "/home/rtx3090x3/nhu_dcase/mat-SED"
os.chdir(root)
sys.path.append(root)

from recipes.maestro.maestro_setting import *
from recipes.maestro.train import Trainer
...
if __name__ == "__main__":
    ...
    LabelDict = get_labeldict_maestro()

    # set encoder
    encoder = get_encoder(LabelDict, configs)

    # get dataset
    train_dataset, valid_dataset, test_dataset, batch_sampler = get_datasets(configs,
                                                                             encoder,
    ...
    if not configs['generals']["test_only"]:
        ...
        # load existed model
        if configs['generals']["finetune_desed_model"]:
            params_dict = torch.load(configs['generals']["best_paths"])
            params_dict = {k: v for k, v in params_dict.items() if "classifier." not in k}
            trainer.net.load_state_dict(params_dict, strict=False)

        # load existed model
        elif configs['generals']["load_from_existed_path"]:
            trainer.net.load_state_dict(torch.load(configs['generals']["best_paths"]))
        ...
        for epoch in range(configs['training']["n_epochs"]):
            ...
            if (not epoch % configs["generals"]["validation_interval"]) or (epoch
                ...
                if val_metrics > best_val_metrics:
                    ...
                    my_logger.logger.info("New best model at epoch {0}!".format(epoch))
