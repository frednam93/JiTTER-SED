...
class PaSST_SED(SEDModel):
    ...
    def decoder_step(self, x, other_dict):
        ...
        if self.mlm:
            if self.mlm_tool.multitask:
                x, x2, mask_id_seq, mask_shuffle = self.mlm_tool.setence_mask(x, self.mask_token)
                other_dict["multitask_mask"] = mask_shuffle
    ...
    def forward(self,
        ...
        if self.mlm:
            x = self.mlm_mlp(x)
            if self.mlm_tool.multitask:
                other_dict["multitask_pred"] = self.mlm_mlp(other_dict["multitask_pred"])
